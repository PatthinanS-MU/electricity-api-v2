...
app.get("/api/usage/history/:province", (req, res) => { // Endpoint is overlapping with users history, consider changing to /api/usage/history/province/:province
  const { province } = req.params;
  const data = loadData("electricity_usages_en.json");
  const result = data.filter(
    (d) => d.province_name.toLowerCase() === province.toLowerCase(),
  );
  res.json({ error: "Endpoint is overlapping with users history, consider changing to /api/usage/history/province/:province" });
});
...
// Step 6: Integrating Automated Testing (The "Quality Gate") 
